...
def translate_srt():
    ...
    input_path = input("Enter the full path of the .srt file: ").strip()
    
    if not os.path.isfile(input_path) or not input_path.lower().endswith('.srt'):
        print(f"Error: '{input_path}' is not a valid .srt file.")
        return
